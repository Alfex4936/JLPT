...
JLPT N5~N1 어휘 8,631개를 자동으로 넘기는 **오프라인 단일 페이지 앱**. 한국인 학습자 한 명을 위해 만들었다.
...
- 핵심 후크는 **한국 한자음**이다. `問題=문제`, `雑談=잡담`처럼 한자음이 한국어 단어와 그대로 겹치는 항목이 3,909개(45%)다. 이 카드에는 `한자음 = 한국어` 배지가 붙고, 사용자는 외울 게 없다.
...
5. **UI 텍스트는 한국어.**
6. **기본 동작을 바꾸지 않는다.** 사용자가 명시적으로 요구했다: 새 기능은 옵션으로 넣고 기본값은 지금까지의 동작(전체 재생 8,631 셔플, 15초, 채점 UI 숨김)을 유지한다.
...
jlpt-vocab-api(어휘) + KANJIDIC2(한자음)
        ...
        ▼
   base.json ◀── tools/add-words.js ◀── tools/gap-words.json (누락 302개)
        │
        ├─ 청크 TSV 65개 ─▶ 번역 에이전트 ─▶ out/v2-*.jsonl
...
| `tools/add-words.js` | `gap-words.json` 을 base.json 에 덧붙이고 `gap-pNN` 청크 생성. 기존 id 는 건드리지 않는다 |
...
**기준값 8건** — 전부 정상이며, 목록에 없던 단어가 새로 뜨면 그게 의심 대상이다:
...
| 化する(かする) | 활용형 — 예문은 化した |
| タンパク質(たんぱくしつ) · ハラハラ(はらはら) | 실제 표기가 가타카나인데 읽기 필드는 히라가나다. 예문은 `タンパクしつ`·`ハラハラ` 로 옳게 쓰여 있다 |
...
- **더 큰 JLPT 어휘 목록은 존재하지 않는다.** 2010년 개편으로 공식 出題基準 발행이 끊겨서, 공개된 N1~N5 목록은 전부 tanos.co.uk(Jonathan Waller) 한 곳 파생이다 — jamsinclair·elzup·Bluskyo·jisho·JLPT Sensei 다 같은 뿌리다. "더 찾아봐" 요청에 다른 저장소를 뒤지는 건 시간 낭비다. 대신 **원천이 흘린 행을 되찾는 게 실익**이다: tanos 의 `足; 脚` 이형태, `～回` 접사, `(かん)` 품사 주석, `パート (タイム)` 분할 표기를 처리하다 302개가 사라져 있었고 거기에 顔·父·母 같은 N5 핵심어가 있었다. 목록은 `tools/gap-words.json`, 대조 상대는 [stephenmk/yomitan-jlpt-vocab](https://github.com/stephenmk/yomitan-jlpt-vocab)(같은 tanos 를 JMdict 로 정규화한 판)과의 **합집합**이다 — 이쪽도 顔·父·母 가 빠져 있어서 한쪽만 믿으면 안 된다.
