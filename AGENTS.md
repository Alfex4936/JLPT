...
JLPT N5~N1 어휘 8,336개를 자동으로 넘기는 **오프라인 단일 페이지 앱**. 한국인 학습자 한 명을 위해 만들었다.
...
- 핵심 후크는 **한국 한자음**이다. `問題=문제`, `雑談=잡담`처럼 한자음이 한국어 단어와 그대로 겹치는 항목이 3,848개(46%)다. 이 카드에는 `한자음 = 한국어` 배지가 붙고, 사용자는 외울 게 없다.
...
5. **UI 텍스트는 한국어.**
6. **기본 동작을 바꾸지 않는다.** 사용자가 명시적으로 요구했다: 새 기능은 옵션으로 넣고 기본값은 지금까지의 동작(전체 재생 8,336 셔플, 15초, 채점 UI 숨김)을 유지한다.
...
기준값: 총 8,336 / 배지 3,848 / 나머지 전부 0.
「예문-단어 불일치」는 1건이 정상이다 — 居る(おる) 는 예문이 활용형 「おります」라 검사식이 못 잡는다.
