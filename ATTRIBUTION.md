...
- **등급이 갈린다. かな 131음은 무료 등급(Unpaid Services)에서 만들었고, 기사 396클립과 단어 3,718클립은
  ...
  기사 음원은 한 편도 끝내지 못한다.
- **완료 상태**: かな 131/131, 기사 396/396(46편 전부), 단어 8,017/8,017. 단어 덱 9,543항목과
  한자 2,142자 전부가 음원을 가진다 — 두 모드 어디서도 기기 TTS 로 떨어지지 않는다.
  한자 음독·훈독(2,788종)과 예문(N5+N4 1,368문장)은 생성 중이다.
...
- 예문은 **N5·N4 만** 만든다(`assets/audio/ex/<단어 id>.opus`, `tools/build-example-audio.js`).
  문장은 단어처럼 20개씩 묶어 무음으로 자를 수 없다 — 문장 안의 、 휴지가 문장 사이 쉼과 길이가 겹쳐
  경계를 정할 수 없다. 그래서 문장 하나가 요청 하나이고, 전 급수로 넓히면 9,437요청에 **258MB** 다
  (읽기 기사 클립 평균 28KB 실측 기준). git 히스토리는 지워지지 않으므로 저장소가 6배가 되고 되돌릴 수 없다.
  N5+N4 는 1,368문장 37MB 로 감당되는 크기다. 나머지 급수의 예문은 기기 TTS 로 읽는다.
- 예문 파일명이 내용이 아니라 **단어 id** 인 이유: 문장은 길고 구두점이 있어 파일명이 될 수 없고,
  해시를 쓰면 앱과 빌더가 같은 함수를 계속 맞춰야 한다. `w.i` 는 9,543항목에서 유일해 충돌이 구조적으로 없다.
